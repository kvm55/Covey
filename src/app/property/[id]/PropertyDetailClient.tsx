...
interface PropertyDetailClientProps {
  propertyId: string;
  initialScenarios: UnderwritingScenarioRow[];
  initialProperty: SupabaseProperty | null;
}
...
export default function PropertyDetailClient({
  ...
  const [scenarios, setScenarios] = useState<UnderwritingScenarioRow[]>(initialScenarios);
  const [supabaseProperty, setSupabaseProperty] = useState<SupabaseProperty | null>(
    initialScenarios.length > 0 ? null : initialProperty
  );
  const [loading, setLoading] = useState(false);
  const [activeScenarioId, setActiveScenarioId] = useState<string | null>(() => {
    if (initialScenarios.length > 0) {
      const primary = initialScenarios.find(s => s.is_primary) || initialScenarios[0];
      return primary.id;
    }
    return null;
  });
  ...
  // ── localStorage migration (client-only, runs once) ─────────
  useEffect(() => {
    if (scenarios.length > 0) return; // already have scenarios from server
    if (!supabaseProperty) return; // no property either
    ...
    async function migrateLocalStorage() {
      try {
        ...
        if (newScenario) {
          ...
          setSupabaseProperty(null);
          localStorage.removeItem(`covey-deal-${propertyId}`);
        }
      } catch {
        // Fall through — keep showing supabaseProperty view
      }
    }

    migrateLocalStorage();
  }, [propertyId, scenarios.length, supabaseProperty]);

  // ── Scenario actions ────────────────────────────────────────
  const reloadScenarios = useCallback(async () => {
    const rows = await fetchScenarios(propertyId);
    setScenarios(rows);
  ...
  }, [propertyId, scenarios, activeScenarioId, reloadScenarios]);
  ...
  if (scenarios.length === 0 && !supabaseProperty) {
    return (
      <div className={styles.container}>
        ...
        <p className={styles.notFoundText}>No property found with ID: <strong>{propertyId}</strong></p>
